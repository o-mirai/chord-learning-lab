...
function updateMidiStatus(isConnected, message) {
  ...
  if (!statusElement) {
    console.warn('midi-status要素が見つかりません');
    return;
  }
...
function checkAnswerAuto() {
  ...
  if (isCorrect) {
    ...
    result.innerHTML = `${UI_MESSAGES.CORRECT_PREFIX} [${currentChord.name}: ${correctNotes.join(', ')}]`;
    ...
    result.innerHTML = `${UI_MESSAGES.INCORRECT_PREFIX} [${currentChord.name}: ${correctNotes.join(', ')}]`;
